...
## Beginner-basics completion

The basics baseline is implemented and tested. Model-role prompts now require **short, beginner-level language**, simple explanations of needed technical terms, honest confidence, evidence-first reasoning, and no invented or copied long-form material. Generic factual questions written as questions are routed to the source-backed research path, while BADGR-specific questions remain eligible for the local RAG/model path. See [docs/BASICS.md](docs/BASICS.md) for setup, the three beginner tests, live commands, PATH installation, and shell-function shortcuts.

The repository includes `bin/badgr` for a short global command. After linking it into `~/.local/bin`, users can run `badgr "your goal"` instead of repeating the Python invocation. The full local suite currently passes **108 tests** with one existing dependency deprecation warning from the FastAPI/httpx test stack.
