...
# Plain factual questions should use the evidence path even when they do not say
# "research" or "search". This keeps broad topics source-backed by default.
FACTUAL_QUESTION_STARTS = (
...
def classify_task(user_goal: str) -> TaskType:
    text = " ".join((user_goal or "").lower().split())
    for task_type, keywords in KEYWORD_MAP.items():
        if any(keyword in text for keyword in keywords):
            return task_type

    # A short factual question is research even when the user does not ask for
    # citations explicitly. Code and planning keywords were already checked
    # above, so this rule does not steal those tasks.
    if (
        text.endswith("?")
        and text.startswith(FACTUAL_QUESTION_STARTS)
        and "badgr" not in text
    ):
