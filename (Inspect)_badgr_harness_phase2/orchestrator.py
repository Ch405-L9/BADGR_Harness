...
def _make_task(*, task_id: str, user_goal: str, task_type: Any, constraints: list[str], expected_output: str, confidence_required: float, status: Any) -> Task:
    ...
    kwargs['confidence_required' if 'confidence_required' in fields else 'confidencerequired'] = confidence_required
    return Task(**kwargs)


def _make_harness_event(*, task: Task, action: str, status: Any, model_used: Optional[str] = None, role_used: Optional[str] = None, validation_passed: Optional[bool] = None, error_message: Optional[str] = None, next_action: Optional[str] = None, details: Optional[Dict[str, Any]] = None, parent_event_id: Optional[str] = None) -> HarnessEvent:
...
def _model_config_by_name(model_name: str, registry: Dict[str, Any]) -> Dict[str, Any]:
    for config in registry.values():
        if _cfg_value(config, 'model_name', 'modelname') == model_name:
            return config
    return {}
...
def _prompt_path(name: str) -> Path:
    if name in PROMPT_NAME_CANDIDATES:
        for candidate in PROMPT_NAME_CANDIDATES[name]:
            ...
            if path.exists():
                return path
        return PROMPTS_DIR / PROMPT_NAME_CANDIDATES[name][0]
...
def build_prompt(task: Task, role_prompt: str, retry_note: str = '') -> str:
    ...
    }.get(task_type, '{"task_type":"general","summary":"...","confidence":0.99,"recommended_action":"...","needs_clarification":false,"clarification_question":null}')
...
def make_event(task: Task, action: str, status: Any, model_used: Optional[str] = None, role_used: Optional[str] = None, validation_passed: Optional[bool] = None, error_message: Optional[str] = None, next_action: Optional[str] = None, details: Optional[Dict[str, Any]] = None, parent_event_id: Optional[str] = None) -> HarnessEvent:
...
def attempt_model(task: Task, model_name: str, prompt_file: str, retry_note: str = '', registry: Optional[Dict[str, Any]] = None) -> ValidationOutcome:
    ...
    timeout_seconds = int(_cfg_value(model_config, 'timeout_seconds', 'timeoutseconds', default=120))
    temperature = float(_cfg_value(model_config, 'temperature', default=0.1))
    raw_output = call_ollama(model_name, prompt, timeout_seconds=timeout_seconds, temperature=temperature)
...
def run_task(user_goal: str) -> Dict[str, Any]:
    ...
    start_event = make_event(task=task, action='task_started', status=_event_status('STARTED'), next_action='route_primary_model', details={'task_type': _task_type_value(task)})

    task_type = _task_attr(task, 'task_type', 'tasktype')
    primary_model = choose_primary_model(task_type, registry)
    primary_event = make_event(task=task, action='primary_model_selected', status=_event_status('STARTED'), model_used=primary_model, role_used='worker', next_action='run_primary_attempt', parent_event_id=_event_id(start_event))

    first_try = attempt_model(task, primary_model, 'worker', registry=registry)
    if first_try.valid:
        _set_task_attr(task, _task_status('SUCCESS'), 'status')
        make_event(task=task, action='primary_attempt_valid', status=_event_status('SUCCESS'), model_used=primary_model, role_used='worker', validation_passed=True, next_action='return_result', parent_event_id=_event_id(primary_event))
        append_report(task, 'success', f'Primary model succeeded: {primary_model}')
        return first_try.data or {}

    retry_event = make_event(task=task, action='primary_attempt_invalid', status=_event_status('RETRY'), model_used=primary_model, role_used='worker', validation_passed=False, error_message=first_try.error, next_action='retry_primary_once', parent_event_id=_event_id(primary_event))

    second_try = attempt_model(task, primary_model, 'worker', retry_note='Retry note: your prior answer failed validation. Return only valid JSON in the exact shape required.', registry=registry)
    if second_try.valid:
        _set_task_attr(task, _task_status('SUCCESS'), 'status')
        make_event(task=task, action='primary_retry_valid', status=_event_status('SUCCESS'), model_used=primary_model, role_used='worker', validation_passed=True, next_action='return_result', parent_event_id=_event_id(retry_event))
    ...
    fallback_event = make_event(task=task, action='fallback_model_selected', status=_event_status('FALLBACK'), model_used=fallback_model, role_used='fallback', validation_passed=False, error_message=second_try.error, next_action='run_fallback_attempt', parent_event_id=_event_id(retry_event))

    fallback_try = attempt_model(task, fallback_model, 'fallback', retry_note='Fallback note: repair the failed attempts and return valid JSON only.', registry=registry)
    if fallback_try.valid:
        _set_task_attr(task, _task_status('SUCCESS'), 'status')
        make_event(task=task, action='fallback_valid', status=_event_status('SUCCESS'), model_used=fallback_model, role_used='fallback', validation_passed=True, next_action='return_result', parent_event_id=_event_id(fallback_event))
    ...
    supervisor_event = make_event(task=task, action='supervisor_selected', status=_event_status('ESCALATED'), model_used=supervisor_model, role_used='supervisor', validation_passed=False, error_message=fallback_try.error, next_action='run_supervisor_attempt', parent_event_id=_event_id(fallback_event))

    supervisor_try = attempt_model(task, supervisor_model, 'supervisor', retry_note='Supervisor note: failed worker attempts require synthesis or a short clarification question.', registry=registry)
    if supervisor_try.valid:
        ...
        if data.get('needs_clarification'):
            ...
            make_event(task=task, action='clarification_required', status=_event_status('NEEDS_CLARIFICATION'), model_used=supervisor_model, role_used='supervisor', validation_passed=True, next_action='ask_human', details={'clarification_question': clarification}, parent_event_id=_event_id(supervisor_event))
            append_report(task, 'needs_clarification', clarification)
            return {'status': 'needs_clarification', 'question': clarification}

        _set_task_attr(task, _task_status('SUCCESS'), 'status')
        make_event(task=task, action='supervisor_valid', status=_event_status('SUCCESS'), model_used=supervisor_model, role_used='supervisor', validation_passed=True, next_action='return_result', parent_event_id=_event_id(supervisor_event))
        append_report(task, 'success', f'Supervisor succeeded: {supervisor_model}')
        return data

    _set_task_attr(task, _task_status('FAILED'), 'status')
    make_event(task=task, action='task_failed', status=_event_status('FAILED'), model_used=supervisor_model, role_used='supervisor', validation_passed=False, error_message=supervisor_try.error, next_action='ask_human', parent_event_id=_event_id(supervisor_event))
